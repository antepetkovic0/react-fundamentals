...
`function Component({ children }) { return <div>{children}</div> }` can be
called as `<Component>Hello World</Component>` or
`<Component children="Hello World" />`

`function Component({ nonChildrenProp }) { return <div>{nonChildrenProp}</div> }`
must be called as `<Component nonChildrenProp="Hello World" />`
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=04%3A%20Creating%20custom%20components&em=ante.petkovic01%40gmail.com).
