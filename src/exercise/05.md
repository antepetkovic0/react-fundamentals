...
_What is it?_ object that is a CSS declaration block, and exposes style
information and various style-related methods and properties

_How to access it?_ select target element from the dev tools -> `$0.style`

### Spreading props

spreading props is a **better** way of creating custom components

`function Component({ className, style, ...props })`

with this approach all extra component properties will be applied included
`children`

```html
<Component id="test-id" className="box--test">test box</Component>
```
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=05%3A%20Styling&em=ante.petkovic01%40gmail.com).
