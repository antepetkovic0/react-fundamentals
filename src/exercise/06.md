...
- represtentation of form submit event
- in react it is a type of SyntheticBaseEvent ('copy' of the native DOM event)
- if needed we can access to native one by `event.nativeEvent`

### event.preventDefault()

- prevents default behavior of form submit events (which refreshes the page)
- default behaviour refreshes the page with query params from the form inputs
  (form inputs must have `name` property)

`console.dir(event.target)` - displays an interactive list of the properties of
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=06%3A%20Forms&em=ante.petkovic01%40gmail.com).
